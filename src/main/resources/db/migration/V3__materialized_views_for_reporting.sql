...
SELECT
    ...
    SUM(rfc.amount) AS total_fixed_cost,
    SUM(rm.unit_cost * ri.quantity * COALESCE(cf.factor, 1.0)) + COALESCE(SUM(rfc.amount), 0) AS total_recipe_cost,
    r.yield_quantity,
    r.yield_unit,
    (SUM(rm.unit_cost * ri.quantity * COALESCE(cf.factor, 1.0)) + COALESCE(SUM(rfc.amount), 0)) / NULLIF(r.yield_quantity, 0) AS cost_per_serving,
    r.created_at,
    r.updated_at,
    r.is_active
FROM recipes r
         ...
         LEFT JOIN conversion_factors cf ON (ri.unit_id = cf.from_unit_id AND rm.purchase_unit_id = cf.to_unit_id)
...
GROUP BY r.id, r.name, r.user_id, u.username, c.name, r.yield_quantity, r.yield_unit, r.created_at, r.updated_at, r.is_active;
...
SELECT
    ...
    c.name AS category_name,
    rm.unit_cost,
    rm.purchase_quantity,
    rm.minimum_stock,
    rm.current_stock,
    mu.code AS unit,
    (rm.purchase_quantity * rm.unit_cost) AS inventory_value,
    CASE
        WHEN rm.purchase_quantity = 0 THEN 'OUT_OF_STOCK'
        WHEN rm.purchase_quantity < rm.minimum_stock THEN 'LOW_STOCK'
        WHEN rm.purchase_quantity > rm.current_stock THEN 'OVERSTOCK'
        ELSE 'NORMAL'
        END AS stock_status,
    CASE
        WHEN rm.minimum_stock > 0 THEN (rm.purchase_quantity / rm.minimum_stock) * 100
        ELSE 100
    ...
    (SELECT unit_cost FROM material_price_history mph WHERE mph.raw_material_id = rm.id ORDER BY changed_at DESC LIMIT 1) AS last_known_price
FROM raw_materials rm
    JOIN users u ON rm.user_id = u.id
    JOIN measurement_units mu ON rm.purchase_unit_id = mu.id
...
SELECT
    ...
    MIN(mph.new_cost) AS min_price_ever,
    MAX(mph.new_cost) AS max_price_ever,
    AVG(mph.new_cost) AS avg_price,
    rm.unit_cost AS current_price,
    (rm.unit_cost - AVG(mph.new_cost)) AS price_variance_from_avg,
    ((rm.unit_cost - AVG(mph.new_cost)) / NULLIF(AVG(mph.new_cost), 0)) * 100 AS price_variance_percent,
    MAX(mph.changed_at) AS last_change_date,
    (SELECT mph2.new_cost FROM material_price_history mph2
...
ORDER BY mph2.changed_at DESC LIMIT 1) AS price_90_days_ago,rm.currency
FROM raw_materials rm
...
SELECT
    ...
    AVG(qi.profit_percentage) AS avg_margin_percent,
    q.is_shareable,
...
GROUP BY q.id, q.quote_number, q.user_id, u.username, q.client_name, q.status,
         q.currency, q.subtotal, q.tax_amount, q.total, q.created_at,
         q.valid_until, q.is_shareable;
...
SELECT
    ...
    rca.total_recipe_cost * (1 + (pm.percentage / 100)) AS selling_price,
    rca.total_recipe_cost * (pm.percentage / 100) AS profit_amount,
    pm.percentage AS profit_margin_percent,
    rca.cost_per_serving,
    rca.cost_per_serving * (1 + (pm.percentage / 100)) AS selling_price_per_serving,
    -- Break-even analysis
    CASE
        WHEN rca.total_fixed_cost > 0 AND (rca.total_recipe_cost * (pm.percentage / 100)) > 0
            THEN CEIL(rca.total_fixed_cost / (rca.total_recipe_cost * (pm.percentage / 100)))
        ELSE 0
...
SELECT
    ...
    (SELECT COUNT(*) FROM raw_materials WHERE user_id = u.id AND purchase_quantity < minimum_stock AND is_active = TRUE) AS low_stock_materials,
    (SELECT SUM(purchase_quantity * unit_cost) FROM raw_materials WHERE user_id = u.id AND is_active = TRUE) AS total_inventory_value,
    -- Recipe metrics
    ...
    (SELECT SUM(total) FROM quotes WHERE user_id = u.id AND status = 'ACCEPTED') AS total_revenue,
